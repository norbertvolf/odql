...
module.exports = (function() {
	...
	OData.prototype.get = function(columnList, entitySetName, limit) {
	...
	OData.prototype.requestOptions = function(options) {
		...
		optionsForRequest.strictSSL = options.strictSSL;
	...
	OData.prototype.connect = function(options) {
		...
		return new Promise(function(resolve, reject) {
			var metadataUrl = `${options.url}/$metadata?format=json`;
	...
	OData.prototype.getEntitySets = function(entitySetName) {
		...
		if (_.isString(entitySetName) && entitySetName.match(/[*]/) === null) {
			entitySets = entitySets.filter((entitySet) => {
				return entitySet.Name.toUpperCase() === entitySetName.toUpperCase();
			});
		} else if (_.isString(entitySetName)) {
			regex = new RegExp("^" + entitySetName.replace("*", ".*") + "$", "i");
			entitySets = entitySets.filter((entitySet) => {
				return entitySet.Name.match(regex) !== null;
			});
		}
		return entitySets;
	};
