...
Cli.error = function() {
	...
	if (args[0] instanceof Error) {
		args[0] = args[0].message + (options.trace && args[0].stack ? "\n" + args[0].stack : "");
	}
...
Cli.prototype.saveHistory = function(history) {
	...
	return new Promise(function(resolve, reject) {
		fs.writeFile(options.historyFile, data, (fileAccessError) => {
			if (fileAccessError) {
...
Cli.prototype.readHistory = function(readlineInstace) {
	return new Promise(function(resolve, reject) {
		var history;
		var historyFileName = options.historyFile;

		if (fs.existsSync(historyFileName)) {
			fs.readFile(options.historyFile, {
				"encoding": "utf-8"
...
Cli.prototype.handlerLine = function(readlineInstance, odata, line) {
	...
	try {
		...
		if (e.offset) {
			Cli.log(new Array(options.prompt.length + e.offset + 1).join("-") + "^\n");
			Cli.log(e.message);
		} else {
			Cli.error(e);
		}
