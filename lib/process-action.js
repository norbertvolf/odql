...
process.get = function(action, Cli, readlineInstance, odata, options) {
	odata.get(action.columnList, action.entitySetName).then((result) => {
		Cli.log(formatter(options).renderEntitySetContent(result.value, odata.getEntitySetDefinition(action.entitySetName)));
		readlineInstance.prompt();
	}).catch((e) => {
		Cli.error(e);
		readlineInstance.prompt();
