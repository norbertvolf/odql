...
/**
 * Printe text parameter as centered to string which size is defined by parameter width
 * @private
 *
 * @param {String} text text which is centered
 * @param {String} width widht of the centered text
 *
...
moduleFormatter.renderCenteredText = function(text, width) {
	var left = Math.round(text.length / 2) + Math.round(width / 2);
	return printf(
		"%-*s",
		printf("%*s", text, left > width ? width : left),
		width
	);
};
